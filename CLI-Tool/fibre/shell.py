...
def launch_shell(args,
    ...
    async def setup_console(console):
        await console.run_code('import sys')
        await console.run_code('superexcepthook = sys.excepthook')
        await console.run_code('def newexcepthook(ex_class,ex,trace):\n'
                            '  if ex_class.__module__ + "." + ex_class.__name__ != "fibre.ChannelBrokenException":\n'
                            '    superexcepthook(ex_class,ex,trace)')
        await console.run_code('sys.excepthook = newexcepthook')

    asyncio.run(setup_console(console))
